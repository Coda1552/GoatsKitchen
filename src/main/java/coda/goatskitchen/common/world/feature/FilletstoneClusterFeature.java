...
public class FilletstoneClusterFeature extends Feature<FilletstoneClusterConfiguration> {
   ...
   public boolean place(FeaturePlaceContext<FilletstoneClusterConfiguration> p_159605_) {
      WorldGenLevel worldgenlevel = p_159605_.level();
      BlockPos blockpos = p_159605_.origin();
      FilletstoneClusterConfiguration clusterConfig = p_159605_.config();
      Random random = p_159605_.random();
      int i = clusterConfig.height.sample(random);
      float f = clusterConfig.wetness.sample(random);
      float f1 = clusterConfig.density.sample(random);
      int j = clusterConfig.radius.sample(random);
      int k = clusterConfig.radius.sample(random);

      for(int l = -j; l <= j; ++l) {
         for(int i1 = -k; i1 <= k; ++i1) {
            double d0 = this.getChanceOfStalagmiteOrStalactite(j, k, l, i1, clusterConfig);
            BlockPos blockpos1 = blockpos.offset(l, 0, i1);
            this.placeColumn(worldgenlevel, random, blockpos1, l, i1, f, d0, i, f1, clusterConfig);
         }
      }
   ...
   private void placeColumn(WorldGenLevel p_159594_, Random p_159595_, BlockPos p_159596_, int p_159597_, int p_159598_, float p_159599_, double p_159600_, int p_159601_, float p_159602_, FilletstoneClusterConfiguration p_159603_) {
      ...
      if (optional.isPresent()) {
         ...
         if (optionalint.isPresent() || optionalint1.isPresent()) {
            ...
            if (optionalint.isPresent() && flag1 && !this.isLava(p_159594_, p_159596_.atY(optionalint.getAsInt()))) {
               ...
               this.replaceBlocksWithFilletstone(p_159594_, p_159596_.atY(optionalint.getAsInt()), k, Direction.UP);
               int l;
            ...
            if (optionalint2.isPresent() && flag2 && !this.isLava(p_159594_, p_159596_.atY(optionalint2.getAsInt()))) {
               ...
               this.replaceBlocksWithFilletstone(p_159594_, p_159596_.atY(optionalint2.getAsInt()), i1, Direction.DOWN);
   ...
   private int getDripstoneHeight(Random p_159613_, int p_159614_, int p_159615_, float p_159616_, int p_159617_, FilletstoneClusterConfiguration p_159618_) {
      if (p_159613_.nextFloat() > p_159616_) {
         return 0;
      } else {
         int i = Math.abs(p_159614_) + Math.abs(p_159615_);
         float f = (float)Mth.clampedMap(i, 0.0D, (double)p_159618_.maxDistanceFromCenterAffectingHeightBias, (double)p_159617_ / 2.0D, 0.0D);
   ...
   private boolean canPlacePool(WorldGenLevel p_159620_, BlockPos p_159621_) {
      ...
      if (!blockstate.is(Blocks.WATER) && !blockstate.is(Blocks.RED_SANDSTONE)) {
   ...
   private void replaceBlocksWithFilletstone(WorldGenLevel p_159589_, BlockPos p_159590_, int p_159591_, Direction p_159592_) {
   ...
   private double getChanceOfStalagmiteOrStalactite(int p_159577_, int p_159578_, int p_159579_, int p_159580_, FilletstoneClusterConfiguration p_159581_) {
